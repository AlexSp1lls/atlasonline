...
var types = require("@sanity/types"), selector_isAtTheStartOfBlock = require("../_chunks-cjs/selector.is-at-the-start-of-block.cjs"), util_reverseSelection = require("../_chunks-cjs/util.reverse-selection.cjs"), util_sliceBlocks = require("../_chunks-cjs/util.slice-blocks.cjs"), selector_getTextBefore = require("../_chunks-cjs/selector.get-text-before.cjs"), selector_getTrimmedSelection = require("../_chunks-cjs/selector.get-trimmed-selection.cjs");
const getActiveAnnotations = (snapshot) => {
  ...
  const key = context.selection && util_reverseSelection.isKeyedSegment(context.selection.anchor.path[0]) ? context.selection.anchor.path[0]._key : void 0, node = key ? context.value.find((block) => block._key === key) : void 0;
  return node && key ? {
    node,
    path: [{
      _key: key
  ...
  if (!anchorBlock)
    return;
  const key = context.selection && util_reverseSelection.isKeyedSegment(context.selection.anchor.path[2]) ? context.selection.anchor.path[2]._key : void 0, node = key ? anchorBlock.node.children.find((span) => span._key === key) : void 0;
  return node && key ? {
    node,
  ...
  if (!selectionStartPoint || !selectionEndPoint)
    return;
  const start = util_reverseSelection.spanSelectionPointToBlockOffset({
    value: context.value,
    selectionPoint: selectionStartPoint
  }), end = util_reverseSelection.spanSelectionPointToBlockOffset({
    value: context.value,
