"use strict";
var types = require("@sanity/types"), selector_isAtTheStartOfBlock = require("./selector.is-at-the-start-of-block.cjs"), util_reverseSelection = require("./util.reverse-selection.cjs"), selector_getTextBefore = require("./selector.get-text-before.cjs"), behavior_core = require("./behavior.core.cjs");
  ...
  const automaticBlockquoteOnSpace = behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const previousInlineObject = selector_isAtTheStartOfBlock.getPreviousInlineObject({
        context
      }), blockOffset = util_reverseSelection.spanSelectionPointToBlockOffset({
        value: context.value,
        selectionPoint: {
          path: [{
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = util_reverseSelection.getTextBlockText(focusTextBlock.node), caretAtTheEndOfQuote = blockOffset.offset === 1, looksLikeMarkdownQuote = /^>/.test(blockText), blockquoteStyle = config.blockquoteStyle?.(context);
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const blockOffset = util_reverseSelection.spanSelectionPointToBlockOffset({
        value: context.value,
        selectionPoint: {
          path: [{
            _key: focusTextBlock.node._key
      ...
      const previousInlineObject = selector_isAtTheStartOfBlock.getPreviousInlineObject({
        context
      }), blockText = util_reverseSelection.getTextBlockText(focusTextBlock.node), markdownHeadingSearch = /^#+/.exec(blockText), level = markdownHeadingSearch ? markdownHeadingSearch[0].length : void 0, caretAtTheEndOfHeading = blockOffset.offset === level;
    ...
    guard: ({
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = util_reverseSelection.getTextBlockText(focusTextBlock.node), defaultStyle = config.defaultStyle?.(context), looksLikeUnorderedList = /^(-|\*)/.test(blockText), unorderedListStyle = config.unorderedListStyle?.(context), caretAtTheEndOfUnorderedList = blockOffset.offset === 1;
